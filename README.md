...
Based on upstream **telegram-send**; see the original [README](https://github.com/rahiel/telegram-send) and [documentation](https://rahiel.github.io/telegram-send/) for full feature coverage.
...
- `--reply-to-sent` — only accept a reply to the message(s) just sent (helpful in busy groups).
- `--quiet` — suppress stderr while waiting (timeout is still reported on stderr).
- `--timeout` — without `--wait-reply`: network read timeout (default 30s). With `--wait-reply`: total wait time (default 300s).

**Groups:** if the bot must see all messages, disable [privacy mode](https://core.telegram.org/bots/features#privacy-mode) in BotFather or mention the bot. **Channels** are not supported for receiving replies.
...
sending to a channel in a non-default configuration: `telegram-send-wait --config
...
We can combine telegram-send-wait with [cron][] to periodically send messages. Here
...
your bot's username when adding it. Then run `telegram-send-wait --configure-channel
...
[upstream send documentation](https://rahiel.github.io/telegram-send/telegram_send.html#send).
...
See [examples/wait_reply_example.py](examples/wait_reply_example.py).
...
See [CONTRIBUTING.md](CONTRIBUTING.md) for the full PyPI checklist (account, token, `uv publish`, tag `v1.0.0`, smoke test with pipx).
